...
export async function POST(request: Request) {
  try {
    ...
    const lead = await prisma.lead.create({ data: { name: String(name).trim(), phone: String(phone).trim(), email: email ? String(email).trim() : null, projectType: String(projectType).trim(), budget: budget ? String(budget).trim() : null, location: location ? String(location).trim() : null, message: String(message).trim() } });
    return NextResponse.json({ success: true, message: "Your inquiry has been received.", leadId: lead.id }, { status: 201 });
  } catch (error) {
