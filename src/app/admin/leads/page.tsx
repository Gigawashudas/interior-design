...
CONTACTED: "bg-blue-500/10 text-blue-600 dark",
QUALIFIED: "bg-purple-500/10 text-purple-600 dark",
CONVERTED: "bg-green-500/10 text-green-600 dark",
LOST: "bg-red-500/10 text-red-600 dark",
};
...
FORM<span className="text-[#F97316]">/</span>SPACE
        ...
        <p className="mt-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-black/40 dark:text-white/40">
          Admin / Leads
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <button
          ...
          className="flex h-9 items-center justify-center gap-2 border border-black/10 px-3 text-xs font-semibold transition-colors hover:border-[#F97316] hover:text-[#F97316] disabled:cursor-not-allowed disabled:opacity-50 dark:border-white/10"
        >
          <RefreshCw
            size={14}
        ...
        <Link
          ...
          className="flex h-9 items-center justify-center gap-2 border border-black/10 px-3 text-xs font-semibold transition-colors hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10"
        >
          <ArrowLeft size={14} />
  ...
  <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-14">
    <div className="border-t-4 border-[#F97316] pt-6">
      ...
      <div className="mt-3 flex flex-col justify-between gap-5 md:flex-row md:items-end">
        <div>
          <h1 className="font-display text-4xl font-medium leading-none tracking-[-0.04em] sm:text-5xl">
            Project inquiries
          </h1>

          <p className="mt-4 max-w-xl text-sm leading-6 text-black/50 dark:text-white/50">
            Manage and follow up with people interested in
            working with FORM/SPACE.
          </p>
        </div>
    ...
    <div className="mt-10 flex flex-col gap-3 border-y border-black/10 py-4 dark:border-white/10 lg:flex-row">
      <div className="relative min-w-0 flex-1">
        ...
        <input
          ...
          className="w-full border border-black/10 bg-transparent py-3 pl-11 pr-4 text-sm outline-none transition-colors placeholder:text-black/30 focus:border-[#F97316] dark:border-white/10 dark:placeholder:text-white/30"
        />
      </div>
      ...
      <select
        ...
        className="border border-black/10 bg-white px-4 py-3 text-sm outline-none focus:border-[#F97316] dark:border-white/10 dark:bg-[#111111]"
      >
      ...
      {(search || statusFilter !== "ALL") && (
        <button
          ...
          className="border border-black/10 px-4 py-3 text-sm font-semibold transition-colors hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10"
        >
          Clear
        </button>
      )}
    ...
    {error && (
      <div className="mt-6 border border-red-500/20 bg-red-500/5 px-4 py-4 text-sm text-red-600 dark:text-red-400">
        {error}
      </div>
    )}
      ...
      <div className="mt-8 border border-black/10 px-6 py-16 text-center dark:border-white/10">
        ...
        {(search || statusFilter !== "ALL") && (
          <button
            type="button"
            onClick={clearFilters}
            className="mt-6 text-sm font-semibold text-[#F97316]"
          >
            Clear filters
          </button>
        )}
          ...
          <table className="w-full border-collapse text-left">
            ...
            <tbody>
              {filteredLeads.map((lead) => (
                <tr
                  key={lead.id}
                  className="group border-b border-black/10 transition-colors last:border-0 hover:bg-black/[0.02] dark:border-white/10 dark:hover:bg-white/[0.02]"
                >
                  ...
                  <td className="px-5 py-5">
                    <button
                      ...
                      className="flex h-9 w-9 items-center justify-center border border-black/10 transition-colors hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10"
        ...
        <div className="mt-8 grid gap-4 lg:hidden">
          {filteredLeads.map((lead) => (
            <button
              ...
              type="button"
              onClick={() => setSelectedLead(lead)}
              className="w-full border border-black/10 p-5 text-left transition-colors hover:border-[#F97316] dark:border-white/10"
            >
  ...
  {selectedLead && (
    ...
    >
      <div
        className="max-h-[90vh] w-full max-w-2xl overflow-y-auto bg-white text-[#111111] dark:bg-[#181818] dark:text-white"
      ...
      >
        ...
        <div className="space-y-7 p-6 sm:p-8">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            <DetailItem
              label="Project type"
              value={selectedLead.projectType}
            />
            ...
            <DetailItem
              ...
              )} at ${formatTime(selectedLead.createdAt)}`}
            />
          </div>
          ...
          <div className="border-t border-black/10 pt-6 dark:border-white/10">
            ...
            <p className="mt-3 text-xs text-black/40 dark:text-white/40">
              Status editing will be connected to the API in
              the next step.
            </p>
          </div>
        </div>
...
{icon}
    ...
    <span className="text-[9px] font-semibold uppercase tracking-[0.16em] text-black/40 dark:text-white/40">
      {label}
    </span>
  </div>
