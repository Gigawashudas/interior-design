...
export default function AdminLeadsPage() {
  ...
  const updateLeadStatus = async (status: LeadStatus) => {
    if (!selectedLead || status === selectedLead.status) {
      return;
    }
    ...
    try {
      ...
      setStatusUpdateError(err instanceof Error ? err.message : "Failed to update lead status.");
    } finally {
    ...
    <main className="min-h-screen bg-white text-[#111111] transition-colors duration-300 dark:bg-[#111111] dark:text-white">
      <header className="sticky top-0 z-40 border-b border-black/10 bg-white/95 backdrop-blur dark:border-white/10 dark:bg-[#111111]/95">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 sm:px-6 lg:px-8">
          <div>
            <Link href="/" className="text-lg font-bold tracking-tighter transition-colors hover:text-[#F97316] sm:text-xl">
      ...
      <div className="mx-auto max-w-7xl px-5 py-8 sm:px-6 sm:py-10 lg:px-8 lg:py-14">
        <div className="border-t-4 border-[#F97316] pt-6">
          ...
          <div className="mt-3 flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <h1 className="font-display text-4xl font-medium leading-none tracking-[-0.04em] sm:text-5xl">Project inquiries</h1>

              <p className="mt-4 max-w-xl text-sm leading-6 text-black/50 dark:text-white/50">Manage and follow up with people interested in working with FORM/SPACE.</p>
            </div>
        ...
        <div className="mt-10 grid grid-cols-2 border-t border-black/10 dark:border-white/10 sm:grid-cols-3 lg:grid-cols-6">
          ...
          <StatCard label="Contacted" value={counts.contacted} active={statusFilter === "CONTACTED"} onClick={() => setStatusFilter(statusFilter === "CONTACTED" ? "ALL" : "CONTACTED")} />

          <StatCard label="Qualified" value={counts.qualified} active={statusFilter === "QUALIFIED"} onClick={() => setStatusFilter(statusFilter === "QUALIFIED" ? "ALL" : "QUALIFIED")} />

          <StatCard label="Converted" value={counts.converted} icon={<CheckCircle2 size={17} />} active={statusFilter === "CONVERTED"} onClick={() => setStatusFilter(statusFilter === "CONVERTED" ? "ALL" : "CONVERTED")} />
        ...
        {loading ? (
          ...
          <>
                ...
                <tbody>
                  {filteredLeads.map((lead) => (
                    <tr key={lead.id} className="group border-b border-black/10 transition-colors last:border-0 hover:bg-black/[0.02] dark:border-white/10 dark:hover:bg-white/[0.02]">
                      ...
                      <td className="px-5 py-5">
                        <button type="button" onClick={() => openLead(lead)} className="flex h-9 w-9 items-center justify-center border border-black/10 transition-colors hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10" aria-label={`View ${lead.name}`}>
            ...
            <div className="mt-8 grid gap-4 lg:hidden">
              {filteredLeads.map((lead) => (
                <button key={lead.id} type="button" onClick={() => openLead(lead)} className="w-full border border-black/10 p-5 text-left transition-colors hover:border-[#F97316] dark:border-white/10">
      ...
      {selectedLead && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5" onClick={closeLead}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto bg-white text-[#111111] dark:bg-[#181818] dark:text-white" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between border-b border-black/10 p-6 dark:border-white/10 sm:p-8">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl font-medium tracking-tight">{selectedLead.name}</h2>
              ...
              <button type="button" onClick={closeLead} disabled={updatingStatus} className="text-sm font-medium transition-colors hover:text-[#F97316] disabled:opacity-50">
                Close
              </button>
            </div>
            ...
            <div className="space-y-7 p-6 sm:p-8">
              <div className="grid gap-5 sm:grid-cols-2">
                ...
                {selectedLead.email && <DetailItem icon={<Mail size={16} />} label="Email" value={selectedLead.email} href={`mailto:${selectedLead.email}`} />}

                <DetailItem icon={<MapPin size={16} />} label="Location" value={selectedLead.location || "Not specified"} />

                <DetailItem label="Project type" value={selectedLead.projectType} />

                <DetailItem label="Budget" value={selectedLead.budget || "Not specified"} />
              ...
              <div className="border-t border-black/10 pt-6 dark:border-white/10">
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
                  {statuses.map((status) => {
                    const isSelected = selectedLead.status === status;

                    return (
                      <button key={status} type="button" onClick={() => void updateLeadStatus(status)} disabled={updatingStatus} className={`border px-3 py-3 text-[10px] font-bold tracking-[0.08em] transition-colors ${isSelected ? "border-[#F97316] bg-[#F97316]/10 text-[#F97316]" : "border-black/10 hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10"} disabled:cursor-not-allowed disabled:opacity-50`}>
                ...
                <p className="mt-3 text-xs text-black/40 dark:text-white/40">Changes are saved immediately.</p>
              </div>
            </div>
          </div>
        </div>
  ...
  const content = (
    <>
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold uppercase tracking-[0.16em] text-black/40 dark:text-white/40">{label}</span>
