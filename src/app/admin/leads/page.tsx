...
export default function AdminLeadsPage() {
  ...
  async function updateLeadStatus(status: LeadStatus) {
    ...
    try {
      ...
      setStatusError(err instanceof Error ? err.message : "Failed to update lead status.");
    } finally {
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 backdrop-blur-sm sm:items-center sm:p-5" onClick={() => setSelectedLead(null)}>
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto bg-white text-[#111111] dark:bg-[#181818] dark:text-white" onClick={(event) => event.stopPropagation()}>
            <div className="flex items-start justify-between border-b border-black/10 p-6 dark:border-white/10 sm:p-8">
              ...
              <button type="button" onClick={() => setSelectedLead(null)} className="text-sm font-medium transition-colors hover:text-[#F97316]">
                Close
              </button>
            </div>
            ...
            <div className="space-y-7 p-6 sm:p-8">
              ...
              <div className="border-t border-black/10 pt-6 dark:border-white/10">
                <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-black/40 dark:text-white/40">Lead status</p>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">
                  {statuses.map((status) => {
                    ...
                    return (
                      <button key={status} type="button" onClick={() => updateLeadStatus(status)} disabled={updatingStatus} className={`border px-3 py-3 text-[9px] font-bold tracking-[0.08em] transition-colors disabled:cursor-not-allowed disabled:opacity-50 ${active ? "border-[#F97316] bg-[#F97316]/10 text-[#F97316]" : "border-black/10 hover:border-[#F97316] hover:text-[#F97316] dark:border-white/10"}`}>
                        {updatingStatus && active ? "UPDATING..." : status}
                      </button>
                    );
                  })}
                </div>
                ...
                <p className="mt-3 text-xs text-black/40 dark:text-white/40">Select a status to update this lead.</p>
              </div>
            </div>
          </div>
        </div>
